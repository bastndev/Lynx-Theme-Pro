...
export async function install(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    ...
    void vscode.window.showInformationMessage(t('lynx.blur.install.success.win'), { title: t('lynx.blur.btn.restart') }).then(msg => { if (msg) {void promptRestart();} });
  } catch {
    writer.cleanup();
...
export async function uninstall(context: vscode.ExtensionContext): Promise<void> {
  ...
  try {
    ...
    void vscode.window.showInformationMessage(t('lynx.blur.uninstall.success.win'), { title: t('lynx.blur.btn.restart') }).then(msg => { if (msg) {void promptRestart();} });
  } catch {
    writer.cleanup();
  } finally { _installing = false; }
}
